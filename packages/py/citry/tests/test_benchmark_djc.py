...
# tests/test_benchmark_djc.py at commit
...
# Adapted: upstream snapshots the output under the `djc_test` fixture (which
# also resets and repopulates the registry). The components self-register via
# their `@register(...)` decorators at import, so no explicit registration is
# needed here. The page is non-deterministic (naturaltime is relative to now,
# the CSRF token and the per-render `data-djc-id-*` markers are random), so
# this is a structural smoke test: it guards that the benchmarked logic still
# renders the whole page without error.
